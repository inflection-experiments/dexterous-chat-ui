			...
			{#each section.rows as row, rowIdx}
				...
				>
					{#each row as cell, cellIdx}
						<td
							class="px-4 py-3 align-top text-sm text-white/90 {cellIdx === 0
								? 'w-16 text-center font-semibold'
								: cellIdx === 1
									? 'min-w-[150px] font-medium'
									: cellIdx === 2
										? 'min-w-[200px] font-mono text-[#ff6b35]'
										: cellIdx === 3
											? 'w-24 text-center uppercase font-semibold'
											: 'min-w-[300px]'} {cellIdx === 1 && !cell ? 'opacity-50 italic' : ''}"
						>
